...
def test_detect_events():
    ...
    speakerDS, speakerEventDS, speakerFilt, micDS, micEventDS, micFilt = detect_events(speaker_data, mic_data)

    speakerDS_expected = np.array([0.291835, 0.617208])
    speakerEventDS_expected = np.array([30.])
    micDS_expected = np.array([0., 0.699534])
    micEventDS_expected = np.array([30.])
